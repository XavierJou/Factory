...
public interface IDAOCours extends JpaRepository<Cours, Integer> {
	...
	@Query("SELECT DISTINCT c FROM Cours c LEFT JOIN FETCH c.formateur WHERE c.formation.id = :idFormation")
    List<Cours> findAllWithFormateur(@Param("idFormation") Integer idFormation);
